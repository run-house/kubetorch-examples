...
class DINOv3EmbeddingService:
    ...
    def __init__(self, model_name="vitl16"):
        ...
        self.backbone = DINOv3ViT(model_name=model_name, pretrained=True).to(
            self.device
        )
        self.backbone.eval()
...
def deploy_separate_services():
    ...
    gpu_compute = kt.Compute(
        gpus=1,
        image=kt.Image("nvcr.io/nvidia/pytorch:23.10-py3").pip_install(
            ["transformers", "pillow", "soxr"]
        ),
    ).autoscale(min_scale=0, max_scale=10, concurrency=50, metric="concurrency")

    embedding_service = kt.cls(DINOv3EmbeddingService).to(
        gpu_compute, init_args={"model_name": "vitl16"}
    )
