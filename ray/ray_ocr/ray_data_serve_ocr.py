...
class OCRServeDeployment:
    ...
    async def __call__(self, request: Dict) -> Dict:
        ...
        try:
            ...
            async for request_output in self.engine.generate(model_input, sampling_params, request_id):
...
def process(input_dir, scale, batch_size, creds_path, output_dir):
    ...
    deployment = OCRServeDeployment.options(name="ocr-deployment", num_replicas=scale).bind()
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Ray Data OCR processor with continuous batching")
