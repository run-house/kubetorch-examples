...
from . import envvars, logger as blog
...
def _benchmark_fn(f, repeats):
    ...
    durations = sorted(
        [start_event[i].elapsed_time(end_event[i]) for i in range(repeats)]
    )
...
def benchmark_tflops(
    name, flops, run_func=None, baseline=None, key_metric=False, repeats=1000
):
